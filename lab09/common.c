...
long long int sum_simd(unsigned int vals[NUM_ELEMS]) {
  ...
  for (unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
    ...
    for (unsigned int i = 0; i < NUM_ELEMS / 4 * 4; i += 4) {
      ...
      __m128i mask = _mm_cmpgt_epi32(vals_loaded, _127);
      __m128i vals_masked = _mm_and_si128(vals_loaded, mask);
      sums = _mm_add_epi32(sums, vals_masked);
    }
...
long long int sum_simd_unrolled(unsigned int vals[NUM_ELEMS]) {
  ...
  for (unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
    ...
    for (unsigned int i = 0; i < NUM_ELEMS / 16 * 16; i += 16) {
      ...
      mask = _mm_cmpgt_epi32(vals_loaded, _127);
      vals_masked = _mm_and_si128(vals_loaded, mask);
      sums = _mm_add_epi32(sums, vals_masked);
